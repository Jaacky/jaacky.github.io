...
function createKillZone(event) {
  ...
  for (var j = GAME.ANTS.colony.length - 1; j >= 0; j--) {
    if (touching(tempKillZone, GAME.ANTS.colony[j])) {
      ...
      GAME.ANTS.fadeAnt(
        GAME.ANTS.deadAnts[GAME.ANTS.deadAnts.length - 1],
        1000
      );
...
var killZone = function (x, y) {
  ...
  this.c1 = function () {
    return [this.x_min(), this.y_min()];
  };
  this.c2 = function () {
    return [this.x_max(), this.y_min()];
  };
  this.c3 = function () {
    return [this.x_min(), this.y_max()];
  };
  this.c4 = function () {
...
GAME.core = {
  ...
  render: function () {
    ...
    for (var j = 0; j < GAME.ANTS.colony.length; j++) {
      drawAnt(
        GAME.ANTS.colony[j].x,
        GAME.ANTS.colony[j].y,
        GAME.ANTS.colony[j].colour,
        GAME.ANTS.colony[j].fadeStage,
        GAME.ANTS.colony[j].rotation
      );
    }
    for (var k = 0; k < GAME.ANTS.deadAnts.length; k++) {
      drawAnt(
        GAME.ANTS.deadAnts[k].x,
        GAME.ANTS.deadAnts[k].y,
        GAME.ANTS.deadAnts[k].colour,
        GAME.ANTS.deadAnts[k].fadeStage,
        GAME.ANTS.deadAnts[k].rotation
      );
    }
  },
};
...
GAME.ANTS = {
  ...
  findPie: function (ant) {
    ...
    var distance = Math.sqrt(
      (x - pie_x) * (x - pie_x) + (y - pie_y) * (y - pie_y)
    );
  ...
  approachTarget: function (ant) {
    ...
    (target_x = ant.target[0]), (target_y = ant.target[1]);
    vx = target_x - x;
    vy = target_y - y;
...
function touching(obj1, obj2) {
  ...
  return (
    (inbetween(x_min, x_max, c1[0]) && inbetween(y_min, y_max, c1[1])) ||
